...
const checkBalance = async (pool, provider, pgClient, alertsTS) => {
	try {
		...
		if (replacedString.includes('{threshold')) {
			replacedString = replacedString.replace('{threshold}', threshold);
		}
		...
		if (filterPools?.length) {
			const data = await Promise.all(
				filterPools.map((pool) =>
					checkBalance(pool, provider, pgClient, alertsTS)
				)
			);
